...
int clog_GET(clog_HTTP *data) {
    ...
    data->data = CLOG_INTERNAL_GET_BUF;
...
void clog_AddBody(clog_HTTP *data, char *body) {
    ...
    clog_AddHeader(data, "Content-Length", content_length);
    
    // Set body
    memcpy(CLOG_INTERNAL_GET_BUF + data->offset, "\r\n", 2);
    memcpy(CLOG_INTERNAL_GET_BUF + data->offset + 2, body, len);
    data->offset += 2 + len;
}
